...
let addOptionsToRightPlaces = (
  questionBody: array<QuestionCard.bodyPart>,
  options: array<Room.answerOption>,
) => {
  let counter = ref(0)
  Array.map(questionBody, bodyPart =>
    switch bodyPart {
    | QuestionCard.Option((i, _)) => QuestionCard.Option((
        i,
        switch options[counter.contents] {
        | Some(answerOption) => {
...
let make = () => {
  ...
  <div className={styles["result-stage"]}>
    ...
    <div className={styles["button-wrapper"]}>
      <Button label={`Продолжить`} onClick=finishRound disabled={!isUserRoundLeader} />
    </div>
  </div>
}
